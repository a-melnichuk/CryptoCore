...
public extension NEMCore.Transaction.Transfer {
    func sign(privateKey: inout Data) throws -> Signed {
        guard let publicKeyBytes = NEMCore.publicKey(privateKey: privateKey),
        ...
        guard let recipient = Base58.decode(self.recipient) else {
            throw NEMCore.TransactionError.addressDecodingFailed
        }
        if let sender = NEMCore.address(publicKey: publicKeyBytes, scheme: scheme) {
        ...
        if let balance = self.balance, amount == balance {
            guard balance > 0 else {
                throw NEMCore.TransactionError.invalidBalance
            }
            guard fee < amount else {
                throw NEMCore.TransactionError.amountExceedsBalance
            }
            amount -= fee
        }
        
        let type = NEMCore.Transaction.Kind.transfer
        ...
        if let assetId = assetId, !assetId.isEmpty {
            guard let decodedAssetId = Base58.decode(assetId) else {
                throw NEMCore.TransactionError.assetIdDecodingFailed
            }
        ...
        if let feeAssetId = feeAssetId, !feeAssetId.isEmpty {
            guard let decodedFeeAssetId = Base58.decode(feeAssetId) else {
                throw NEMCore.TransactionError.feeAssetIdDecodingFailed
            }
        ...
        guard let signatureBytes = NEMCore.sign(serializedTx, privateKey: &privateKey),
        ...
        guard let idBytes = Crypto.blake2b256(serializedTx),
            ...
            throw NEMCore.TransactionError.transactionIdGenerationFailed
        }
...
extension NEMCore.Transaction.Transfer.Signed: Encodable {
