...
public struct NEMCore {
    ...
    public static func address(publicKey: Data, scheme: UInt8 = 87 /* W */) -> String? {
        let count = Int(PTC_NEM_ADDRESS_BYTE_COUNT)
        let address = Crypto.callCrypto(publicKey, outCount: count) {
            ptc_nem_address($0, scheme, $2)
        }
    ...
    public static func valid(address: String, scheme: UInt8 = 87 /* W */) -> Bool {
        let result = address.withCString { ptc_nem_address_valid($0, scheme) }
