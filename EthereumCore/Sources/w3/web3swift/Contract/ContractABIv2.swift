...
struct ContractV2: ContractProtocol {
    ...
    /// Init with json ABI
    init(_ abiString: String, at address: Address? = nil) throws {
    ...
    init(abi: [ABIv2.Element], at: Address) {
        _abi = abi
        address = at
    }
    ...
    func deploy(bytecode: Data, args: Any..., extraData: Data = Data(), options: Web3Options?) throws -> EthereumTransaction {
    ...
    ///   - options: Transaction options
    /// - Returns: Transaction and its hash
    func deploy(bytecode: Data, parameters: [Any], extraData: Data = Data(), options: Web3Options?) throws -> EthereumTransaction {
    ...
    ///   - options: Transaction options
    /// - Returns: Transaction
    func method(_ name: String, args: Any..., extraData: Data = Data(), options: Web3Options?) throws -> EthereumTransaction {
    ...
    ///   - options: Transaction options
    /// - Returns: Transaction
    func method(_ name: String, parameters: [Any], extraData: Data = Data(), options: Web3Options?) throws -> EthereumTransaction {
        var to: Address
        let options = self.options.merge(with: options)
