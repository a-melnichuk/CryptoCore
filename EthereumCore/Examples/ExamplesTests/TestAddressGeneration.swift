...
class TestAddressGeneration: XCTestCase {
    ...
    func testAddressGeneration() {
        do {
            let path = Bundle(for: type(of: self)).url(forResource: "eth", withExtension: "txt")!
            ...
            for (i, data) in dataModels.enumerated() {
                ...
                guard let publicKey = EthereumCore.publicKey(privateKey: privateKey.raw) else {
                ...
                guard let address = EthereumCore.address(publicKey: publicKey) else {
