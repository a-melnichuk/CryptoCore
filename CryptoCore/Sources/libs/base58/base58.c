...
{
    if (ver == NULL || ver_sz < 1)
        return false;
    size_t buf_sz = ver_sz + datasz + 0x20;
    uint8_t* buf = calloc(buf_sz, sizeof(uint8_t));
    if (buf == NULL)
        return false;
    memcpy(buf, ver, ver_sz);
    memcpy(buf + ver_sz, data, datasz);
    uint8_t* hash = &buf[ver_sz + datasz];
    bool success;
    if (!(success = my_dblsha256(hash, buf, datasz + ver_sz)))
        goto cleanup;
    success = b58enc(b58c, b58c_sz, buf, ver_sz + datasz + 4);
cleanup:
    memset(buf, 0, buf_sz);
    free(buf);
    return success;
